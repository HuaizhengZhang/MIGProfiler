...
for BATCH_SIZE in "${BATCH_SIZES[@]}"; do
    ...
    python client/block_inference_cv.py -b "${BATCH_SIZE}" -m "${MODEL_NAME}" -n "${NUM_TEST_BATCHES}" -i "${GPU_ID}" \
      -dbn "${EXP_SAVE_DIR}" --report-suffix "client1"  > /dev/null 2>&1 &
    CLIENT1_PID=$!

    echo 'Start client 2'
    python client/block_inference_cv.py -b "${BATCH_SIZE}" -m "${MODEL_NAME}" -n "${NUM_TEST_BATCHES}"  -i "${GPU_ID}" \
      -dbn "${EXP_SAVE_DIR}" --report-suffix "client2" > /dev/null 2>&1 &
    CLIENT2_PID=$!

    echo 'Start client 3'
    python client/block_inference_cv.py -b "${BATCH_SIZE}" -m "${MODEL_NAME}" -n "${NUM_TEST_BATCHES}"  -i "${GPU_ID}" \
      -dbn "${EXP_SAVE_DIR}" --report-suffix "client3" > /dev/null 2>&1 &
    CLIENT3_PID=$!

    echo 'Start profiling client 0'
    python client/block_inference_cv.py -b "${BATCH_SIZE}" -m "${MODEL_NAME}" -n "${NUM_TEST_BATCHES}" -i "${GPU_ID}" \
      -dbn "${EXP_SAVE_DIR}" --report-suffix "client0"
