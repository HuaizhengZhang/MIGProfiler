#! /usr/bin/env bash
GPU_ID=0
# obtain GPU instance UUID from nvidia-smi -L
...
for BATCH_SIZE in "${BATCH_SIZES[@]}"; do
    ...
    python client/block_inference.py --dry-run  -b "${BATCH_SIZE}" -m "${MODEL_NAME}" -n 1048575  \
      --device-id "${GPU_INSTANCE1_UUID}" -i "${GPU_ID}" -gi 0  > /dev/null 2>&1 &
    CLIENT1_PID=$!
    ...
    python client/block_inference.py -b "${BATCH_SIZE}" -m "${MODEL_NAME}" -n "${NUM_TEST_BATCHES}" \
      --device-id "${GPU_INSTANCE0_UUID}" -i "${GPU_ID}" -gi 1  -dbn "${EXP_SAVE_DIR}/batch_size"
